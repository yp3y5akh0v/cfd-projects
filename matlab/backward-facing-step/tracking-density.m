...
density = zeros(nxLaplace, nyLaplace);
densityPrev = zeros(nxLaplace, nyLaplace);
...
density(:, nyLaplace) = 0;
density(1 : nstepx - 1, nstepy) = 0;
density(nstepx, 2 : nstepy - 1) = 0;
density(nstepx + 1 : nxLaplace, 1) = 0;
density(nxLaplace, :) = 0;
density(nstepx, nstepy) = 0;    
density(nstepx, 1) = 0;
density(1 : nstepx - 1, 1 : nstepy - 1) = 0;  
...
for i = 2 : nxLaplace - 1
    ...
    for j = j : nyLaplace - 1
        ki1 = 0.5 * (densityPrev(i + 1, j) + densityPrev(i, j));
        ki2 = 0.5 * (densityPrev(i - 1, j) + densityPrev(i, j));
        kj1 = 0.5 * (densityPrev(i, j + 1) + densityPrev(i, j));
        kj2 = 0.5 * (densityPrev(i, j - 1) + densityPrev(i, j));
        density(i, j) = densityPrev(i, j) - dt * dxi * (u(i, j) * ki1 - u(i - 1, j) * ki2) ...
            - dt * dyi * (v(i, j) * kj1 - v(i, j - 1) * kj2) ...
            + dt * dxi^2 * nu * (densityPrev(i + 1, j) - 2 * densityPrev(i, j) + densityPrev(i - 1, j)) ...
            + dt * dyi^2 * nu * (densityPrev(i, j + 1) - 2 * densityPrev(i, j) + densityPrev(i, j - 1));
    end
end

dt = min(0.25 * max(dx, dy)^2 / nu, 2 * nu / maxuv);

img = imagesc(xm, ym, density');
colorbar;
axis equal;
set(gca,'YDir','normal');
